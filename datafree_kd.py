...
# Data Free
parser.add_argument('--method', default='nayer', choices=['nldf'])
...
parser.add_argument('--save_dir', default='run/c100w402w162-nayer', type=str)
...
# CDF
parser.add_argument('--log_tag', default='c100w402w162-nayer-ep120')
parser.add_argument('--epochs', default=120, type=int, metavar='N',
                    help='number of total epochs to run')
parser.add_argument('--warmup', default=20, type=int, metavar='N',
                    help='which epoch to start kd')

parser.add_argument('--g_steps', default=40, type=int, metavar='N',
                    help='number of iterations for generation')
parser.add_argument('--g_life', default=10, type=int,
                    help='meta gradient: is maml or reptile')
parser.add_argument('--g_loops', default=2, type=int, metavar='N', help='which epoch to start kd')
parser.add_argument('--gwp_loops', default=10, type=int, metavar='N', help='which epoch to start kd')
...
parser.add_argument('--student', default='wrn16_2')
parser.add_argument('--dataset', default='cifar100', choices=['cifar10', 'cifar100', 'tiny_imagenet', 'imagenet'])
...
parser.add_argument ('--ov_weight', type = float, default = 5, help = 'learning rate of balance')
...
parser.add_argument('--synthesis_batch_size', default=400, type=int,
                    metavar='N',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    student = registry.get_model(args.student, num_classes=num_classes, args = args)
    teacher = registry.get_model(args.teacher, num_classes=num_classes, pretrained=True, args = args).eval()
    ...
    if args.dataset != 'imagenet':
        print(teacher.load_state_dict(torch.load('checkpoints/pretrained/%s_%s.pth' % (args.dataset, args.teacher),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.warmup:
            ...
            # del vis_image
            # del vis_name
            train(synthesizer, [student, teacher], criterion, optimizer, args, epoch)  # kd_steps
        tm = time.time() - tm
...
# do the distillation
def train(synthesizer, model, criterion, optimizer, args, epoch):
    global time_cost
    ...
    for i in range(args.kd_steps):
        ...
        with args.autocast():
            ...
            pre_features = student.pre_features
            pre_weight1 = student.pre_weight1

            weight1, pre_features, pre_weight1 = weight_learner(s_feat, pre_features, pre_weight1, args, epoch, i)
            # weight1 = weight1.repeat(1, 100).detach()
            student.pre_features.data.copy_(pre_features)
            student.pre_weight1.data.copy_(pre_weight1)

            # criterion(s_out, t_out.detach()) * weight1

            loss_s = criterion(s_out, t_out.detach()).sum(1).view(1, -1).mm(weight1).view(1)
