...
parser.add_argument('--lr_decay_milestones', default="80,120,160", type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        _best_ckpt = "./checkpoints/scratch/"
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                and args.rank % ngpus_per_node == 0):
            if epoch % 10 == 0:
                save_checkpoint(model.state_dict(), is_best, os.path.join(_best_ckpt, "model_{}.pth".format(epoch)))
            if is_best:
                save_checkpoint(model.state_dict(), is_best, os.path.join(_best_ckpt, "model_best.pth"))
